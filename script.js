...
function drawIndividual(canvas, d, isP) {
    ...
    let qty = parseFloat(document.getElementById("qtySpec").value) || 1;
    let area = (d.w * d.h * qty).toFixed(2);
    let rate = parseFloat(d.rate) || 0;
    let amount = (area * rate).toFixed(2);

    // Update the UI Area input automatically
    document.getElementById("areaSpec").value = area;

    ctx.textAlign = "left"; ctx.font = "bold 12px Arial"; let sX = 15; let sY = y + hB * 304.8 * scale + 40; let mW = canvas.width - 30; ctx.fillStyle = "#0f172a"; ctx.fillText("TAG NO / S.NO: ", sX, sY); let idLW = ctx.measureText("TAG NO / S.NO: ").width;
    ...
    let sps = [
        ...
        { l:"QUANTITY: ", v:qty}, { l: "MESH OPTION: ", v: d.mesh },
